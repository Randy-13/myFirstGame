...
int Melee::getNowHealthValue()
{
    return this->nowHealthValue;
}
...
void Melee::updateCost(int cost)
{
    this->cost = cost;
}
...
void Melee::updateMoveDirection(QChar dir)
{
 ...
 bool Melee::updateNowHealthValue(int num)
 {
     int tpValue = this->nowHealthValue;
     if(tpValue + num >= 0)
     {
         this->nowHealthValue += num;
         return true;
     }else{
         return false;
     }
